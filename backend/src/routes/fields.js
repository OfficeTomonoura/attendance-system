...
router.put('/reorder/all', async (req, res) => {
    try {
        ...
        await prisma.$transaction(items.map((item) => prisma.attendanceField.update({
            where: { id: item.id },
...
// 勤怠項目に紐づくグループ一覧取得
router.get('/:id/salary-groups', async (req, res) => {
    try {
        const id = req.params.id;
...
// 勤怠項目のグループ紐付け一括更新
router.put('/:id/salary-groups', async (req, res) => {
    try {
        const id = req.params.id;
