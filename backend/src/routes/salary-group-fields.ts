...
// グループに紐づく項目一覧取得
router.get('/:salaryGroupId', async (req: Request, res: Response) => {
  try {
    const salaryGroupId = req.params.salaryGroupId as string;
    ...
    for (const cf of commonFields) {
      if (!existingFieldIds.has(cf.id)) {
        merged.push({
          ...
          attendanceField: cf as any
        } as any);
      }
    }

    // attendanceField の displayOrder でソート（マスタの順序と一致させる）
    merged.sort((a, b) => (a as any).attendanceField.displayOrder - (b as any).attendanceField.displayOrder);
...
// グループ別項目設定の保存（一括更新）
router.post('/:salaryGroupId', async (req: Request, res: Response) => {
  try {
    const salaryGroupId = req.params.salaryGroupId as string;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const field of fields) {
        if (commonFieldIds.has(field.attendanceFieldId)) continue; // 共通項目は紐付けテーブルに保存しない
